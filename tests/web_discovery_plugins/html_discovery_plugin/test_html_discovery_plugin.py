...
from statick_tool.plugins.discovery.html_discovery_plugin import \
    HTMLDiscoveryPlugin
...
def test_html_plugin_found():
    ...
    assert any(plugin_info.plugin_object.get_name() == 'html' for
    ...
    assert any(plugin_info.name == 'HTML Discovery Plugin' for
               plugin_info in manager.getPluginsOfCategory("Discovery"))
