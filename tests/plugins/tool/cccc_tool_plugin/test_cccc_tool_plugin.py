...
def test_cccc_tool_plugin_parse_missing_names():
    ...
    output_file = os.path.join(os.path.dirname(__file__), 'valid_package', 'cccc-missing-names.xml')
    with open(output_file) as f:
        output = xmltodict.parse(f.read())
...
def test_cccc_tool_plugin_scan_empty_calledprocesserror(mock_subprocess_check_output):
    ...
    mock_subprocess_check_output.side_effect = subprocess.CalledProcessError(1, '', output=b"mocked error")
    issues = ctp.scan(package, 'level')
    assert not issues
