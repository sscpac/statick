...
def test_cpplint_tool_plugin_scan_valid():
    ...
    assert len(issues) == 4
    assert issues[2].filename == os.path.join(os.path.dirname(__file__), 'valid_package', 'test.c')
...
def test_cpplint_tool_plugin_parse_valid():
    ...
    ctp = setup_cpplint_tool_plugin()
    output = "{}:14: Redundant blank line at the end of a code block should be deleted. " \
...
def test_checkforexceptions_true():
    ...
    assert CpplintToolPlugin.check_for_exceptions(mm)
    mm.group.side_effect = (
        lambda i: "cfg/cpp/Config.h" if i == 1 else "build/storage_class" if i == 4 else False
    )
    assert CpplintToolPlugin.check_for_exceptions(mm)
