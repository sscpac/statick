...
def test_lizard_tool_plugin_scan_valid():
    ...
    assert issues[0].message == "func has 52 NLOC, 16 CCN, 143 token, 0 PARAM, 69 length"


def test_lizard_tool_plugin_parse_valid():
    """Verify that we can parse the normal output of lizard."""
    ltp = setup_lizard_tool_plugin()
    output = "{}:1: warning: func has 22 NLOC, 18 CCN, 143 token, 0 PARAM, 69 length" \
    ...
    assert issues[0].issue_type == 'warning'
    assert issues[0].severity == '5'
    assert issues[0].message == "func has 22 NLOC, 18 CCN, 143 token, 0 PARAM, 69 length"
