...
def test_cmakelint_tool_plugin_found():
    ...
    assert(any(plugin_info.plugin_object.get_name() == 'cmakelint' for
               plugin_info in manager.getPluginsOfCategory("Tool")))
    # While we're at it, verify that a plugin is named CMake Tool Plugin
    assert(any(plugin_info.name == 'cmakelint Tool Plugin' for
               plugin_info in manager.getPluginsOfCategory("Tool")))
...
def test_cmakelint_tool_plugin_nonexistent_file():
    ...
    cmltp = setup_cmakelint_tool_plugin()
    package = Package('invalid_package', os.path.join(os.path.dirname(__file__),
                                                      'invalid_package'))
