...
from statick_tool.plugins.tool.shellcheck import ShellcheckToolPlugin
...
def test_shellcheck_tool_plugin_found():
    ...
    assert any(
        plugin.get_name() == "shellcheck" for _, plugin in list(plugins.items())
    )
...
@mock.patch("statick_tool.plugins.tool.shellcheck.subprocess.check_output")
