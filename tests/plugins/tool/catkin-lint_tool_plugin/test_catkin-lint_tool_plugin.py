...
from statick_tool.plugins.tool.catkin_lint_tool_plugin import \
    CatkinLintToolPlugin
...
def test_catkin_lint_tool_plugin_found():
    ...
    assert any(plugin_info.plugin_object.get_name() == 'catkin_lint' for
               plugin_info in manager.getPluginsOfCategory("Tool"))
    # While we're at it, verify that a plugin is named Yamllint Tool Plugin
    assert any(plugin_info.name == 'Catkin Lint Tool Plugin' for
               plugin_info in manager.getPluginsOfCategory("Tool"))
...
def test_catkin_lint_tool_plugin_parse_valid():
    ...
    assert issues[0].message == "use ${PROJECT_NAME} instead of 'custom_pkg' " \
    ...
    issues = cltp.parse_output(package, output)
    assert issues[0].message == "missing test_depend on 'rostest' " \
                                "(I can't really tell if this applies for " \
                                "package.xml or CMakeLists.txt. Make sure to " \
                                "check both for this issue)"
    ...
    issues = cltp.parse_output(package, output)
    assert issues[0].message == "unconfigured build_depend on " \
    ...
    issues = cltp.parse_output(package, output)
    assert issues[0].message == "include_directories() needs missing directory '/include'"
