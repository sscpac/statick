...
def test_console_reporting_plugin_found():
    ...
    assert any(plugin_info.plugin_object.get_name() == 'print_to_console' for
               plugin_info in manager.getPluginsOfCategory("Reporting"))
    # While we're at it, verify that a plugin is named C Discovery Plugin
    assert any(plugin_info.name == 'Print To Console Reporting Plugin' for
               plugin_info in manager.getPluginsOfCategory("Reporting"))
...
def test_console_reporting_plugin_report_cert(capsys):
    ...
    assert output == ['Tool tool_a: 1 unique issues', '  test.txt:1: tool_a:type: This is a test (MEM50-CPP) [1]', '1 total unique issues']
...
def test_console_reporting_plugin_report_nocert(capsys):
    ...
    captured = capsys.readouterr()
    output = captured.out.splitlines()
    assert output == ['Tool tool_a: 1 unique issues', '  test.txt:1: tool_a:type: This is a test [1]', '1 total unique issues']
