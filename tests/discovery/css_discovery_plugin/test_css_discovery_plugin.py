...
from statick_tool.plugins.discovery.css_discovery_plugin import CSSDiscoveryPlugin
...
def test_css_plugin_found():
    ...
    assert any(
        plugin_info.plugin_object.get_name() == "css"
    ...
    assert any(
        plugin_info.name == "CSS Discovery Plugin"
        for plugin_info in manager.getPluginsOfCategory("Discovery")
    )
