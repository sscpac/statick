...
from statick_tool.plugins.tool.markdownlint_tool_plugin import \
    MarkdownlintToolPlugin
...
def test_markdownlint_tool_plugin_scan_valid():
    ...
    package['md_src'] = [os.path.join(os.path.dirname(__file__),
                                      'valid_package', 'test_no_issues.md')]
...
def test_markdownlint_tool_plugin_parse_valid():
    ...
    assert issues[0].message == 'Multiple consecutive blank lines [Expected: 1; Actual: 3]'
...
@mock.patch('statick_tool.plugins.tool.markdownlint_tool_plugin.subprocess.check_output')
